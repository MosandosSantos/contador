"""Validador independente da EFD-Contribuições rascunho gerada pelo app.

Confere codificação, estrutura de registros, contagens do bloco 9 (9900/9990/9999)
e a coerência das consolidações (M200 = soma dos M100; M600 = soma dos M500).
Não substitui o PVA oficial.

Uso: python scripts/validar_efd_contribuicoes.py [caminho_do_arquivo.txt]
"""
import sys
...
from core.efd_contribuicoes import gerar, CAMPOS_MINIMOS  # noqa: E402
...
def validar(caminho_arquivo: Path) -> list:
    ...
    # 9990 = linhas do bloco 9 (do 9001 ao 9990, inclusive).
    ...
    # 9999 = total de linhas do arquivo (incluindo a própria).
    if linhas:
    ...
    # Consolidações: M200 = ΣM100 e M600 = ΣM500 (posição 3 = valor da contribuição).
    for cabeca, detalhe in (('M200', 'M100'), ('M600', 'M500')):
        total = 0
        for l in linhas:
            if l.startswith(f'|{detalhe}|'):
                total += _int(l.strip('|').split('|')[3])
        consolidado = [l for l in linhas if l.startswith(f'|{cabeca}|')]
        if consolidado:
            informado = _int(consolidado[0].strip('|').split('|')[2])
            if informado != total:
                erros.append(f'{cabeca} declara {informado}, mas a soma dos {detalhe} é {total}.')
        elif total:
            erros.append(f'Há {detalhe} detalhados, mas {cabeca} não foi emitido.')
    return erros
...
def main():
    if len(sys.argv) > 1:
        ...
        raise SystemExit('Informe o caminho do arquivo: python scripts/validar_efd_contribuicoes.py arquivo.txt')
    erros = validar(alvo)
    if erros:
